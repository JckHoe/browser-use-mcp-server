FROM --platform=linux/arm64 ghcr.io/astral-sh/uv:bookworm-slim AS builder
...
# Install system packages, Playwright dependencies, fonts, etc.
RUN apt-get update && \
...
# Copy Python and app from builder
...
# Install Playwright (chromium only) with dependencies
